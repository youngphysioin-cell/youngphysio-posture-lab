...
const HISTORY_KEY = "youngphysio.posture.history.v1";
...
export default function App() {
  ...
  return (
    <div className="clinic-bg min-h-screen">
      ...
      {/* ============ header ============ */}
      <header className="no-print sticky top-0 z-50 border-b border-line-soft bg-cream/90 backdrop-blur-md">
        <div className="mx-auto flex h-[68px] max-w-[1160px] items-center gap-3 px-4 sm:px-6">
          <a href="#top" className="flex items-center gap-3">
            <LogoMark size={38} />
            <div className="leading-none">
              <div className="font-display text-[17px] font-bold tracking-tight text-ink">
                Young <span className="text-[var(--color-brand)]">Physio</span>
              </div>
              <div className="mt-1 text-[10.5px] font-bold uppercase tracking-[0.14em] text-faint">Professional posture screening</div>
            </div>
          </a>
          <div className="ml-auto flex items-center gap-2.5">
            <span className="hidden items-center gap-2 rounded-full border border-line bg-card px-3 py-1.5 text-[11.5px] font-bold text-muted sm:flex">
              <span
                className={`h-2 w-2 rounded-full ${engine === "loading" ? "anim-pulse-soft" : ""}`}
                style={{ background: engine === "ready" ? "var(--color-scan)" : engine === "error" ? "var(--color-risk)" : engine === "loading" ? "var(--color-info)" : "var(--color-line)" }}
              />
              {engine === "ready" ? "Analysis model ready" : engine === "loading" ? "Preparing model" : engine === "error" ? "Model offline" : "AI Posture Analysis"}
            </span>
            {report && (
              <button onClick={() => window.print()} className="btn btn-outline !min-h-[42px] !px-4 !text-[13.5px]">
                <IconPrinter size={16} /> <span className="hidden sm:inline">Print report</span>
              </button>
            )}
          </div>
        </div>
      ...
      <main id="top" className="mx-auto max-w-[1160px] px-4 pb-16 sm:px-6">
          ...
          <section className="no-print grid items-center gap-10 pb-4 pt-10 sm:pt-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
            <Reveal>
              <div className="eyebrow flex items-center gap-2">
                <IconHeartPulse size={15} /> Young Physio · Digital screening
              </div>
              <h1 className="mt-3.5 font-display text-[34px] font-bold leading-[1.08] tracking-tight text-ink sm:text-[46px]">
                AI-assisted<br />Posture <span className="text-[var(--color-brand)]">Screening</span>
              </h1>
              <p className="mt-5 max-w-[52ch] text-[16px] leading-relaxed text-muted">
                Get a detailed posture screening from the comfort of your home. Capture your posture and receive a structured analysis of
                key alignment patterns — with a personalised exercise plan to support them.
              </p>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <button onClick={() => scrollTo("step-1")} className="btn btn-primary">
                  Begin your screening <IconArrowRight size={17} />
                </button>
                <button onClick={() => runDemo(view)} className="btn btn-outline">Try a demo scan</button>
              </div>
              <p className="mt-4 text-[12.5px] font-medium text-faint">Takes about 2 minutes · No account needed</p>
            </Reveal>

            <Reveal delay={120}>
              <div className="relative overflow-hidden rounded-[26px] border border-line-soft bg-sand p-6 shadow-[var(--shadow-card)] sm:p-8">
                <div className="dot-field pointer-events-none absolute inset-0" aria-hidden />
                <div className="relative flex items-center justify-center">
                  <FigureSide className="h-[300px] sm:h-[340px]" />
                </div>
                <div className="anim-floaty absolute left-5 top-8 rounded-xl border border-line-soft bg-card px-3 py-2 shadow-sm">
                  <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-faint">Head position</div>
                  <div className="value-mono mt-0.5 text-[15px] font-semibold text-scan">+4.2%</div>
                </div>
                <div className="anim-floaty absolute right-5 top-[38%] rounded-xl border border-line-soft bg-card px-3 py-2 shadow-sm" style={{ animationDelay: "1.2s" }}>
                  <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-faint">Neck angle</div>
                  <div className="value-mono mt-0.5 text-[15px] font-semibold text-scan">51.8°</div>
                </div>
                <div className="anim-floaty absolute bottom-9 left-8 rounded-xl border border-line-soft bg-card px-3 py-2 shadow-sm" style={{ animationDelay: "2.1s" }}>
                  <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-faint">Pelvis</div>
                  <div className="mt-0.5 text-[12.5px] font-bold text-ink-soft">Centred ✓</div>
                </div>
                <p className="relative mt-4 text-center text-[11.5px] font-medium text-faint">Illustrative alignment map — your scan maps 33 body points like these</p>
              </div>
            </Reveal>
          </section>
        )}
        ...
        {/* ============ stepper ============ */}
        <div className="no-print sticky top-[68px] z-40 -mx-4 border-b border-line-soft bg-cream/92 px-4 py-3 backdrop-blur-md sm:-mx-6 sm:px-6">
